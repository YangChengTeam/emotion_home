 ...
 */
class LiveLookActivity : BaseActivity(), LiveLookView {
    ...
    private fun initView() {
        ...
        iv_operation.visibility = View.GONE
    ...
    override fun dispatchTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !isMuted) {
    ...
    private fun initListener() {
        ...
        send_txt_msg_submit_container.setOnClickListener { v: View? ->
            var content = live_send_message.text.toString().trim { it <= ' ' }
            if (TextUtils.isEmpty(content)) {
                Toast.makeText(this@LiveLookActivity, "不能发送空消息", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            content = SensitiveWord.filterInfo(this, content)
        ...
        live_send_message.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                ...
                if (TextUtils.isEmpty(result)) {
                    send_txt_msg_submit.setBackgroundColor(ContextCompat.getColor(this@LiveLookActivity, R.color.color_f8f8f8))
                    send_txt_msg_submit.setTextColor(ContextCompat.getColor(this@LiveLookActivity, R.color.color_bdbcbc))
                } else {
                    send_txt_msg_submit.setBackgroundColor(ContextCompat.getColor(this@LiveLookActivity, R.color.main_theme_color))
                    send_txt_msg_submit.setTextColor(ContextCompat.getColor(this@LiveLookActivity, R.color.white))
                }
            }
        })
    ...
    private fun setPraise() {
        ...
        tv_praise_bottom.text = "$praiseCount"
        mLiveInfo?.let {
...
//        loginRoom();
    ...
    private fun logoutRoom() {
        ...
        imManager?.setGroupListener(null)
    ...
    private fun handleUser() {
        ...
        if (uid == 0) {
            userID = YcApplication.getInstance().pesudoUniqueID
    ...
    private fun joinImRoom(liveInfo: LiveInfo) {
        ...
        imManager?.login(userID, GenerateTestUserSig.genTestUserSig(userID), object : V2TIMCallback {
            override fun onError(code: Int, msg: String) {
                ...
                if (code == 8507) {//超过群组成员数的限制
                    val isMini = PreferenceUtil.getInstance().getBooleanValue(roomKey, false)
                    if (!isMini) {
                        loginRoom()
                    } else {
                        enterRoom()
    ...
    private fun showCloseFragment() {
        ...
        closeLiveFragment.setOnCloseListener(object : CloseLiveFragment.OnCloseListener {
            override fun onMinimize() {
                //最小化
                PreferenceUtil.getInstance().setBooleanValue(roomKey, true)
                //                moveTaskToBack(true);
...
//        Log.e(TAG, "joinRoom: " + roomId);
    ...
    private val groupListener: V2TIMGroupListener = object : V2TIMGroupListener() {
        override fun onGroupDismissed(groupID: String, opUser: V2TIMGroupMemberInfo) {
            ...
            if (!isDestroyed) {
                ...
                EventBus.getDefault().post(IndexRefreshEvent("live end"))
                if (!isClose) {
        ...
        override fun onRecvGroupCustomMessage(msgID: String, groupID: String, sender: V2TIMGroupMemberInfo, customData: ByteArray) {
            ...
            when (message.cmdId) {
                ...
                Message.command_muted_msg -> {
                    ...
                    if (TextUtils.equals("true", mutedMsg)) {//禁言
                        fl_speak_container.visibility = View.GONE
                        can_not_speak.visibility = View.VISIBLE
                        isMuted = true

                    } else if (TextUtils.equals("false", mutedMsg)) {//解除禁言
                        fl_speak_container.visibility = View.VISIBLE
                        can_not_speak.visibility = View.GONE
                        isMuted = false
                    }
                }
                Message.command_come_out -> {
                    val msg = message.content
                    val mess = msg.split("-")

                    if (mess.size == 2) {
                        praiseCount = mess[1].toInt()
                        praise_count.text = String.format(getString(R.string.praise_count), praiseCount)
                        tv_praise_bottom.text = "$praiseCount"

                    }

                    online_count.text = String.format(getString(R.string.online_count), mess[0].toInt())
                }
            }
            if (null != chatItem) {
                chatItems.add(chatItem)
    ...
    companion object {
        ...
        fun startActivity(context: Context, roomId: Int, starttime: Long, endtime: Long) {
            val intent = Intent(context, LiveLookActivity::class.java)
