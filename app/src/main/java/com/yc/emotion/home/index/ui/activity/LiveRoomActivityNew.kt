 ...
 */
class LiveRoomActivityNew : BaseActivity(), LiveView, V2TIMCallback {
    ...
    private fun initListener() {
        ...
        send_txt_msg_submit_container.setOnClickListener { v: View? ->
            ...
            if (TextUtils.isEmpty(content)) {
                Toast.makeText(this@LiveRoomActivityNew, "不能发送空消息", Toast.LENGTH_SHORT).show()
        ...
        live_send_message.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                ...
                if (TextUtils.isEmpty(result)) {
                    send_txt_msg_submit.setBackgroundColor(ContextCompat.getColor(this@LiveRoomActivityNew, R.color.color_f8f8f8))
                    send_txt_msg_submit.setTextColor(ContextCompat.getColor(this@LiveRoomActivityNew, R.color.color_bdbcbc))
                } else {
                    send_txt_msg_submit.setBackgroundColor(ContextCompat.getColor(this@LiveRoomActivityNew, R.color.main_theme_color))
                    send_txt_msg_submit.setTextColor(ContextCompat.getColor(this@LiveRoomActivityNew, R.color.white))
                }
            }
        })
        close_live.setOnClickListener { v: View? -> exitRoom() }
...
//        Log.e(TAG, "showCreateRoomSuccess: " + getWx());
    ...
    private val runnable: Runnable = object : Runnable {
        override fun run() {
            ...
            if (System.currentTimeMillis() >= endTime * 1000) {
                if (dialog == null) {
                    dialog = AlertDialog.Builder(this@LiveRoomActivityNew).setTitle("提示")
    ...
    companion object {
        ...
        fun startActivity(context: Context?, liveUser: LiveInfo?) {
            val intent = Intent(context, LiveRoomActivityNew::class.java)
